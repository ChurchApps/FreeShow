...
{:else if $draw !== null}
    {#if tool === "focus"}
        ...
        <div class="focus" style="left: {x}px;top: {y}px;opacity: {settings?.opacity};border-radius: {settings?.radius}%;width: {settings?.size}px;height: {settings?.size}px;box-shadow: 0 0 0 50000px {settings?.color}{settings?.glow ? `, inset 0 0 ${settings?.size / 3}px ${settings?.color}` : ''};" in:fade={{ duration: 150 }}></div>
