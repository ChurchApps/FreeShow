<script lang="ts">
  ...
  function mousedown(e: any) {
    ...
    mouse = {
      ...
      offset: {
        x:
          (e.clientX - e.target.closest(".slide").offsetLeft) / ratio -
          target.offsetLeft,
        y:
          (e.clientY - e.target.closest(".slide").offsetTop) / ratio -
          target.offsetTop,
  ...
  $: layout =
    active && $showsCache[active]
      ? $showsCache[active].settings.activeLayout
      : "";
  ...
  function keydown(e: any) {
    ...
    if (
      e.key === "Enter" &&
      e.altKey &&
      (e.target.closest(".item") || e.target.closest(".quickEdit"))
    ) {
      // incorrect editbox
      if (
        e.target.closest(".quickEdit") &&
        Number(e.target.closest(".quickEdit").getAttribute("data-index")) !==
          editIndex
      )
        return;
      ...
      if (!sel?.length || (sel.length === 1 && !Object.keys(sel[0]).length))
        return;
      ...
      lines.forEach((line, i) => {
        if (start > -1 && currentIndex >= start)
          secondLines.push({ align: line.align, text: [] });
      ...
      if (!firstLines.length || !firstLines[0].text.length)
        firstLines = defaultLine;
      if (!secondLines.length) secondLines = defaultLine;

      if (typeof $activeEdit.slide === "number") editIndex = $activeEdit.slide;
      let editItemIndex: number =
        $activeEdit.items[0] ||
        Number(e.target.closest(".editItem").getAttribute("data-index")) ||
        0;
      ...
      // set child
      let parentId =
        slideRef.type === "child" ? slideRef.parent.id : slideRef.id;
    ...
    if (
      !$activeEdit.items.includes(index) ||
          ...
          overlays.update((a: any) => {
            $activeEdit.items.forEach((i: number) => {
              if (a[$activeEdit.id!].items[i])
                a[$activeEdit.id!].items.splice(i, 1);
            });
            return a;
          });
  ...
  function deselect(e: any) {
    ...
    if (
      e.ctrlKey ||
      e.metaKey ||
      e.target.closest(".item") === itemElem ||
      !$activeEdit.items.includes(index) ||
  ...
  function paste(e: any) {
    ...
    let emptySelection: boolean = !sel.filter((a) => Object.keys(a).length)
      .length;

    let caret = { line: 0, pos: 0 };
    sel.forEach((lineSel, i) => {
      if (
        lineSel.start !== undefined ||
        (emptySelection && i >= sel.length - 1)
      ) {
        let pos = 0;
        let pasted = false;
        lines[i].text.forEach(({ value }, j) => {
          ...
          if (
            !pasted &&
            (pos + value.length >= lineSel.start ||
              (emptySelection && j >= lines[i].text.length - 1))
          ) {
            ...
            lines[i].text[j].value =
              value.slice(0, caretPos) +
              clipboard +
              value.slice(caretPos + removeText, value.length);
            pasted = true;
          }
          pos += value.length;
        });
  ...
  function getMirroredItem() {
    ...
    let newSlideRef: any = _show(item.mirror!.show)
      .layouts("active")
      .ref()[0]?.[$activeEdit.slide || 0];
    if (!newSlideRef) return;
    slideId = newSlideRef.id;
    let newItem: any = _show(item.mirror!.show)
      .slides([slideId])
      .items([0])
      .get()[0]?.[0];
    if (!newItem) return;
  ...
  async function getChordPosition(chord: any, line: number) {
    ...
    let lineLetters = item.lines![line].text?.reduce(
      (value, text) => (value += text.value.length),
      0
    );
  ...
  function chordDown(chordData) {
    selected.set({
      id: "chord",
      data: [{ slideId: ref.id, itemIndex: index, ...chordData }],
    });
    chordDrag = chordData;
  ...
  function chordMove(e: any) {
    ...
    let lineLetters = item.lines![closestLine.index].text?.reduce(
      (value, text) => (value += text.value.length),
      0
    );
    ...
    let currentChordIndex = newLines[chordDrag.index].chords.findIndex(
      (a) => a.id === chordDrag.chord.id
    );
    ...
    if (chordDrag.index !== closestLine.index) {
      ...
      // add to new line
      if (!newLines[chordDrag.index].chords)
        newLines[chordDrag.index].chords = [];
  ...
  function addChords() {
    ...
    newLines[0].chords.push({ id: uid(), pos: 0, key: keys[0] });
...
<div
  ...
  style={plain
    ? "width: 100%;"
    : `${item?.style}; outline: ${3 / ratio}px solid rgb(255 255 255 / 0.2);`}
  data-index={index}
...
>
  ...
  {#if item?.lines}
    ...
    >
      ...
      {#if chordsMode}
        <Button class="context #chord" on:click={addChords}>
          <Icon id="add" white />
        </Button>
      {/if}
    </div>
            ...
            {#if line.chords}
              {#each line.chords as chord}
                {#await getChordPosition(chord, i) then pos}
                  <div
                    class="context #chord chord"
                    style={pos}
                    on:mousedown={() => chordDown({ chord, index: i })}
                  >
                    <Button
                      style="padding: 0 15px;"
                      on:click={() => changeKey(chord, i)}
                    >
                      {chord.key}
                    </Button>
                  </div>
    ...
    {#if item.src}
      ...
      {:else}
        <Image
          src={item.src}
          alt=""
          style="width: 100%;height: 100%;object-fit: {item.fit ||
            'contain'};filter: {item.filter};{item.flipped
            ? 'transform: scaleX(-1);'
            : ''}"
        />
    ...
    {#if item.mirror?.show}
      {#key item.mirror?.show}
        ...
        {:then}
          {#if getMirroredItem()}
            <Textbox
              item={getMirroredItem()}
              ref={{ showId: item.mirror.show, slideId, id: ref.id }}
            />
          {/if}
        {/await}
      {/key}
