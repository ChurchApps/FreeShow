...
export function getSlides({ bibles, sorted }) {
    ...
    bibles.forEach((bible, bibleIndex) => {
        ...
        const alignStyle = currentTemplate?.lines?.[1]?.align || currentTemplate?.lines?.[0]?.align || "text-align: left;"
        const textStyle = currentTemplate?.lines?.[1]?.text?.[0]?.style || currentTemplate?.lines?.[0]?.text?.[0]?.style || "font-size: 80px;"

        const emptyItem = {
            lines: [{ text: [], align: alignStyle }],
            style: itemStyle,
            specialStyle: currentTemplate?.specialStyle || {},
        ...
        sorted.forEach((s: any, i: number) => {
            ...
            if (get(scriptureSettings).verseNumbers) {
                ...
                if (i === 0) size *= 1.2
                const verseNumberStyle = textStyle + "font-size: " + size + "px;color: " + (get(scriptureSettings).numberColor || "#919191")
            ...
            if (get(scriptureSettings).redJesus) {
                ...
                if (!jesusWords[0]) {
                    textArray.push({
                        value: removeTags(formatBibleText(text)),
                        style: textStyle,
                    })
                } else if (jesusWords[0]?.[0] > 0) {
                    textArray.push({
                        value: removeTags(formatBibleText(text.slice(0, jesusWords[0][0]))),
                        style: textStyle,
                    })
                }
                ...
                jesusWords.forEach(([start, end], i) => {
                    textArray.push({
                        value: removeTags(formatBibleText(text.slice(start + 2, end - 2))),
                        style: textStyle + redText,
                        customType: "disableTemplate_jw",
                    })
            ...
            if (bibleIndex + 1 >= bibles.length) {
                ...
                indexes.forEach((i) =>
                    addMeta(get(scriptureSettings), joinRange(range), {
                        slideIndex,
                        itemIndex: i,
                    })
                )
        ...
        // add remaining
        if (bibleIndex + 1 >= bibles.length) {
            const remainder = sorted.length % get(scriptureSettings).versesPerSlide
    ...
    function addMeta({ showVersion, showVerse, customText }, range: string, { slideIndex, itemIndex }) {
        ...
        const versions = get(scriptureSettings).combineWithText ? bibleVersions[itemIndex] : bibleVersions.join(" + ")
        const books = get(scriptureSettings).combineWithText ? bibles[itemIndex]?.book : removeDuplicates(bibles.map((a) => a.book)).join(" / ")
        ...
        text.split("\n").forEach((line) => {
            if (!line.trim()) return
            lines.push({
                text: [{ value: line, style: verseStyle }],
                align: alignStyle,
            })
        })
