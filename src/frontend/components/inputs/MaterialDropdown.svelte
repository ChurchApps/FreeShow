<script lang="ts">
    ...
    function handleKeydown(event: KeyboardEvent) {
        ...
        if (event.key === "Enter" || event.key === " ") {
            ...
            else highlightedIndex = (highlightedIndex + 1) % options.length
        } else if (event.key === "ArrowUp") {
            event.preventDefault()
            if (!open) toggleDropdown(true)
            else highlightedIndex = (highlightedIndex - 1 + options.length) % options.length
...
<style>
    ...
    :global(.textfield:focus-within) .underline {
        transform: scaleX(1);
    }
    :global(.textfield:focus-within) label {
        top: 0.25rem;
        font-size: 0.75rem;
        color: var(--secondary);
        font-weight: 500;
