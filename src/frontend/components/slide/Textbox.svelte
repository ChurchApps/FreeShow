<script lang="ts">
  ...
  function getMirroredItem() {
    ...
    if (!ref.type || ref.type === "show") {
      let currentSlideRef: any = _show(
        currentOutput?.out?.slide?.id || "active"
      )
        .layouts("active")
        .ref()[0]
  ...
  {#if lines}
    <div class="align" style={style ? item.align : null}>
      ...
      <div
        class="lines"
        style={smallFontSize
          ? "--font-size: " + (-1.1 * $slidesOptions.columns + 12) * 5 + "px;"
          : null}
        bind:this={textElem}
      >
          ...
          {#if linesStart === null || linesEnd === null || (i >= linesStart && i < linesEnd)}
            ...
            >
              {#each line.text as text}
                <span
                  style="{style ? text.style : ''}{ref.type === 'stage' ||
                  item.auto
                    ? 'font-size: ' + autoSize + 'px;'
