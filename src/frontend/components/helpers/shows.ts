...
/** Shows function */
/** string[] | "active" */
export function _shows(ids: any = []) {
  // await loadShows(ids)
  let shows: any = get(showsCache)
  if ((ids === "active" && get(activeShow) !== null && get(activeShow)!.type === undefined) || get(activeShow)!.type === "show") ids = [get(activeShow)!.id]
  if (!ids.length) ids = Object.keys(shows)
  return {
      ...
      get: (key: null | string = null) => {
        ...
        ids.forEach((id: string, i: number) => {
          ...
          slideIds.forEach((slideId) => {
            if (key) a[i].push(shows[id].slides[slideId][key])
            else a[i].push(shows[id].slides[slideId])
          })
        })
        return a
      },
