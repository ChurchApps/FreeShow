...
export function copy({ id, data }: any = {}, getData = true) {
...
export function deleteAction({ id, data }, type = "delete") {
    console.log("DELETE", id, data)
    if (!deleteActions[id]) return false
    const deleted: any = deleteActions[id](data, type)
...
const selectActions: any = {
    show_drawer: () => {
        ...
        const showsList: string[] = Object.keys(get(shows)).filter((id: any) => !get(shows)[id].private)
    ...
    edit_items: () => {
        ...
        if (!get(activeEdit).type || get(activeEdit).type === "show") {
            ...
            const items = _show().slides([editSlide.id]).get()[0].items
    ...
    slide: () => {
        ...
        newSelection = ref.map((_: any, index: number) => ({
            index,
            showId: get(activeShow)?.id,
        }))
    ...
    folder: () => {
        const newSelection: any[] = Object.keys(get(folders)).map((id) => ({
            type: "folder",
            id,
        }))
    ...
    show: () => {
        ...
        const newSelection: any[] = get(projects)[get(activeProject)!]?.shows.map((a, index) => ({
            ...a,
            name: a.name || removeExtension(getFileName(a.id)),
            index,
        }))
    ...
    overlay: () => {
        ...
        let newSelection: any[] = Object.entries(get(overlays)).map(([id, a]) => ({
            ...a,
            id,
        }))
    ...
    template: () => {
        ...
        let newSelection: any[] = Object.entries(get(templates)).map(([id, a]) => ({
            ...a,
            id,
        }))
        if (selectedCategory !== "all") newSelection = newSelection.filter((a) => (selectedCategory === "unlabeled" ? !a.category : a.category === selectedCategory))
...
const copyActions: any = {
    ...
    slide: (data: any, fullGroup = false) => {
        ...
        const backgrounds = layouts.filter((a) => a.background)
        const showMedia = _show().get().media
        backgrounds.forEach((layoutData) => {
            const bgId = layoutData.background
            const bg = showMedia[bgId]
            if (!bg) return
            media[bgId] = bg
        })
...
const pasteActions: any = {
    item: (data: any) => {
        ...
        if (get(activeEdit).id) {
            if (get(activeEdit).type === "overlay") {
                const items = clone(get(overlays)[get(activeEdit).id!].items || [])
                ...
                history({
                    ...
                    location: { page: "edit", id: "overlay_items" },
                })
                return
            }

            if (get(activeEdit).type === "template") {
                const items = clone(get(templates)[get(activeEdit).id!].items || [])
                data.forEach((item: any) => {
                    items.push(clone(item))
                })
                history({
                    ...
                    newData: { key: "items", data: items },
                    oldData: { id: get(activeEdit).id },
                    location: { page: "edit", id: "template_items" },
                })
                return
            }
        ...
        const ref = _show().layouts("active").ref()[0][get(activeEdit).slide!]
        const items: any[] = []
        data.forEach((item: any) => {
            items.push(clone(item))
        })
    ...
    slide: (data: any, { index }: any = {}) => {
        ...
        const copiedIds: string[] = data.slides.map((a: any) => a.id)
        // let childs: any[] = []
    ...
    overlay: (data: any) => {
        data?.forEach((slide: any) => {
            ...
            history({
                id: "UPDATE",
                newData: { data: newSlide },
                location: { page: "drawer", id: "overlay" },
            })
        })
    },
    template: (data: any) => {
        data?.forEach((slide: any) => {
            ...
            history({
                id: "UPDATE",
                newData: { data: newSlide },
                location: { page: "drawer", id: "template" },
            })
        })
    },
}
...
const deleteActions = {
    item: (data) => {
        ...
        const slide = data.slideId || _show().layouts("active").ref()[0][data.slide].id
        history({
            id: "deleteItem",
            location: {
                page: "edit",
    ...
    audio: (data: any) => {
        ...
        audioPlaylists.update((a) => {
            ...
            data.forEach((song) => {
                const currentSongIndex = songs.findIndex((a) => a === song.path)
    ...
    midi: (data: any) => {
        ...
        ref.forEach((slideRef: any, i: number) => {
            ...
            history({
                id: "SHOW_LAYOUT",
                newData: { key: "actions", data: actions, indexes: [i] },
            })
        })
    ...
    show: (data: any) => {
        ...
        history({
            ...
            newData: {
                key: "shows",
                data: shows.filter((_a, i) => !indexes.includes(i)),
            },
    ...
    layout: (data: any) => {
        if (data.length < _show().layouts().get().length) {
            data.forEach((id: string) => {
                history({
                    id: "UPDATE",
                    newData: { id: get(activeShow)?.id },
                    oldData: { key: "layouts", subkey: id },
    ...
    style: (data: any) => {
        data.forEach(({ id }) => {
            history({
                id: "UPDATE",
                newData: { id },
                location: { page: "settings", id: "settings_style" },
            })
        })
    },
    output: (data: any) => {
        data.forEach(({ id }) => {
            // delete key output
            if (get(outputs)[id]?.keyOutput)
                history({
                    id: "UPDATE",
                    newData: { id: get(outputs)[id].keyOutput },
...
const duplicateActions = {
    ...
    stage: (data: any) => {
        ...
        history({
            id: "UPDATE",
            newData: { data: clone(stage) },
            location: { page: "stage", id: "stage" },
        })
    },
    layout: (data: any) => {
        ...
        history({
            id: "UPDATE",
            newData: { key: "layouts", subkey: uid(), data: newLayout },
            oldData: { id: get(activeShow)?.id },
            location: { page: "show", id: "show_layout" },
        })
    },
    folder: (data: any) => {
        ...
        folders.update((a) => {
            data.forEach((folder) => {
                const id = folder.id
                folder = a[id]
                const parent = Object.values(a).find((a) => a.parent === folder.id) || folder.parent
                const newId = uid()
            ...
            function duplicateFolder(oldParent: string, newParent: string) {
                let folderList: Folder[] = Object.entries(a).map(([id, folder]) => ({
                    id,
                    ...folder,
                }))
                ...
                const projectList = removeDeleted(keysToID(get(projects))).filter((a) => a.parent === oldParent)
    ...
    theme: (data: any) => {
        data.forEach(({ id }) => {
            ...
            history({
                ...
                newData: {
                    data: theme,
                    replace: { default: false, name: name + " 2" },
                },
                location: { page: "settings", id: "settings_theme" },
            })
        })
    ...
    style: (data: any) => {
        data.forEach(({ id }) => {
            ...
            history({
                id: "UPDATE",
                newData: { data: style, replace: { name: style.name + " 2" } },
                oldData: { id },
                location: { page: "settings", id: "settings_style" },
            })
        })
    ...
    output: (data: any) => {
        data.forEach(({ id }) => {
            ...
            history({
                id: "UPDATE",
                newData: { data: output, replace: { name: output.name + " 2" } },
                oldData: { id },
                location: { page: "settings", id: "settings_output" },
            })
        })
...
function historyDelete(id, data, { updater } = { updater: "" }) {
    ...
    data.forEach((a: any) =>
        history({
            id,
            newData: { id: a.id || a },
            location: { page: get(activePage) as any, id: updater || undefined },
        })
    )
}
...
async function duplicateShows(selected: any) {
    ...
    selected.forEach(({ id }: any) => {
        ...
        history({
            ...
            newData: {
                data: show,
                remember: { project: id === "show" ? get(activeProject) : null },
            },
