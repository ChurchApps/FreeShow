...
import { confirmCustom, waitForPopupData } from "../../utils/popup"
...
const deleteActions = {
  ...
  // Handle folder deletion with option to delete all projects inside
  folder: async (data: any) => {
    const folderIds = data.map((item: any) => item.id || item)

    // Check if any folder has projects or subfolders inside (recursively)
    const hasContent = folderIds.some((folderId: string) => hasProjectsInFolder(folderId))

    // If folder has content, show confirmation popup
    if (hasContent) {
      popupData.set({ folderIds })
      const result = await waitForDeleteFolderPopup()

      if (!result) return // User cancelled

      if (result === "folder_and_projects") {
        // Delete all projects and subfolders inside, then delete the folders
        deleteAllFolderContents(folderIds)
      }
      // If "folder_only", fall through to normal deletion (which moves projects to parent)
    }

    // Delete the folders (this will move any remaining projects to parent folder)
    historyDelete("UPDATE", data, { updater: "project_folder" })
  },
...
/**
 * Wait for the delete folder popup to return a result
 * @returns "folder_only" | "folder_and_projects" | undefined (if cancelled)
 */
async function waitForDeleteFolderPopup(): Promise<string | undefined> {
  return await waitForPopupData("delete_folder")
}

/**
 * Check if a folder has any projects or subfolders inside (recursively)
 * @param folderId - The folder ID to check
 * @returns true if folder has content (projects or subfolders)
 */
function hasProjectsInFolder(folderId: string): boolean {
  // Check for direct projects in this folder
  const hasProjects = Object.values(get(projects)).some((project: any) => project.parent === folderId)
  if (hasProjects) return true

  // Check for subfolders - if any exist, the folder has content
  const hasSubfolders = Object.values(get(folders)).some((folder: any) => folder.parent === folderId)
  return hasSubfolders
}

/**
 * Recursively delete all projects and subfolders inside the given folders
 * @param folderIds - Array of folder IDs to delete contents from
 */
function deleteAllFolderContents(folderIds: string[]) {
  const foldersToProcess = [...folderIds]
  const allProjectsToDelete: string[] = []
  const allSubfoldersToDelete: string[] = []

  // Collect all projects and subfolders recursively
  while (foldersToProcess.length > 0) {
    const currentFolderId = foldersToProcess.pop()!

    // Find projects in this folder
    Object.entries(get(projects)).forEach(([projectId, project]: [string, any]) => {
      if (project.parent === currentFolderId) {
        allProjectsToDelete.push(projectId)
      }
    })

    // Find subfolders and add them to process list
    Object.entries(get(folders)).forEach(([subfolderId, folder]: [string, any]) => {
      if (folder.parent === currentFolderId) {
        allSubfoldersToDelete.push(subfolderId)
        foldersToProcess.push(subfolderId)
      }
    })
  }

  // Delete all collected projects
  if (allProjectsToDelete.length > 0) {
    allProjectsToDelete.forEach((projectId) => {
      history({ id: "UPDATE", newData: { id: projectId }, location: { page: "show", id: "project" } })
    })
  }

  // Delete all collected subfolders (in reverse order to delete children first)
  if (allSubfoldersToDelete.length > 0) {
    allSubfoldersToDelete.reverse().forEach((subfolderId) => {
      history({ id: "UPDATE", newData: { id: subfolderId }, location: { page: "show", id: "project_folder" } })
    })
  }
}
