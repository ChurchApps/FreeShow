<script lang="ts">
    ...
    const stageLayouts = sortByName(keysToID($stageShows)).filter((a) => profile[a.id] !== "none" && Object.values($stageShows[a.id]?.items).some((a) => a.currentOutput?.source !== $currentOutputSettings))
