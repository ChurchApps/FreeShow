<script lang="ts">
  ...
  export let maxWidth: number = defaultWidth * 3
  ...
  const conditions = {
    ...
    bottom: (e: any) =>
      e.clientY < e.target.closest(".panel")?.offsetTop + e.target.closest(".panel")?.offsetParent.offsetTop + handleWidth && e.offsetY <= handleWidth && e.offsetY >= 0,
  }
