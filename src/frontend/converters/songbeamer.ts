...
export function convertSongbeamerFiles({ files = [], category = "Songbeamer", translationMethod = TranslationMethod.MultiLine, encoding = "utf8" }: any) {
    ...
    const tempShows: { id: string; show: ShowObj }[] = []
    files.forEach(({ name, content }) => {
        ...
        const show: ShowObj = convertSongbeamerFileToShow(name, content, settings)
        tempShows.push({
            id: uid(),
            show: show,
        })
...
function parseMetadata(text: string, encoding: BufferEncoding = "utf8"): SongbeamerMetadata {
    ...
    text.split("\n").forEach((line: string) => {
        ...
        switch (parts[0]) {
            case "LangCount":
                const langCount = Number.parseInt(parts[1])
            ...
            case "Key":
                ...
                break
            case "Tempo":
                const tempo = Number.parseInt(parts[1])
...
function slideTagToGroup(line: string): {
    group: string | null
    ...
    if (parts.length > 1) {
        groupNumber = Number.parseInt(parts[1])
    ...
    for (const section of sections) {
        ...
        for (let lineText of lines) {
            ...
            if (langOverwrite === null) {
                ...
                const langNumber = Number.parseInt(langOverwrite[2])
                if (!langOverwrite[1]) {
                    ++language
                }
                if (!isNaN(langNumber)) {
...
function createSlidesMultipleLanguagesPerSlide(
    ...
    slides: { [key: ID]: Slide }
    layout: SlideData[]
    groupSlides: Map<string, SlideData>
} {
