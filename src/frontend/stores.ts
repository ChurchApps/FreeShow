...
export const aiScriptureTranscript: Writable<{ text: string; startMs: number; endMs: number; language?: string; music?: boolean; utteranceEnd?: boolean }[]> = writable([])
export const aiScriptureInterim: Writable<string> = writable("") // the open utterance's unstable tail - display only, shown greyed
