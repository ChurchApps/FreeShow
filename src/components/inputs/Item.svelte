...
<button on:click={() => activeProject.set(id)} on:dblclick={() => {projectView.set(false); activeShow.set({id: $projects[id].shows[0].id, index: 0, type: $projects[id].shows[0].type})}} class:active>
  <Icon name="file" />
