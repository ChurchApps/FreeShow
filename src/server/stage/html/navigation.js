...
async function loadSlideContent(
  ...
  // Check cache first
  if (
    slideContentCache.has(
      slideId
    )
  ) {
    return slideContentCache.get(
      slideId
    )
  }
  ...
  try {
    ...
    if (
      ...
      const content =
        newSlideContainer.innerHTML
      // Cache the content
      slideContentCache.set(
        slideId,
        content
      )
      return content
...
async function fetchSlideTimerInfo(
  slideId
) {
  try {
