...
export class ChumsConnect {
  ...
  public static async apiRequest(data: ChumsRequestData): Promise<any> {
    ...
    return new Promise((resolve) => {
      ...
      httpsRequest(apiUrl, data.endpoint, data.method || "GET", headers, data.data || {}, (err, result) => {
        if (err) {
          console.log("Could not get data", apiUrl, data.endpoint)
  ...
  private static async authenticate(scope: ChumsScopes): Promise<ChumsAuthData> {
    ...
    return new Promise((resolve) => {
      ...
      const URL = `${CHUMS_API_URL}/login?returnUrl=` + encodeURIComponent(
        `/oauth?client_id=${this.clientId}&redirect_uri=${encodeURIComponent(redirect_uri)}&response_type=code&scope=${scope}`
      )
      openURL(URL)
    })
  }
