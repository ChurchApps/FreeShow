...
function runTest(config, options) {
    return new Promise((resolve, reject) => {
        if (!options.silent) {
            console.log(`${colors.cyan}Starting: ${config.description}${colors.reset}`)
        }
        ...
        child.on("close", code => {
            if (code === 0) {
                if (!options.silent) {
                    console.log(`${colors.green}✅ ${config.description} completed successfully${colors.reset}`)
                }
                resolve()
            } else {
                if (!options.silent) {
                    console.log(`${colors.red}❌ ${config.description} failed with code ${code}${colors.reset}`)
                }
        ...
        child.on("error", error => {
            if (!options.silent) {
                console.log(`${colors.red}❌ Error running ${config.description}: ${error.message}${colors.reset}`)
            }
            reject(error)
        })
    })
...
async function runAllTests(options) {
    ...
    for (const testType of testOrder) {
        try {
            await runTest(testConfigs[testType], options)
        } catch (error) {
            console.log(`${colors.red}❌ Test suite ${testType} failed${colors.reset}`)
            if (options.verbose) {
...
async function main() {
    ...
    if (options.testType === "all") {
        console.log(`${colors.cyan}Running complete test suite...${colors.reset}\n`)
        ...
        if (config) {
            ...
            console.log(`${colors.red}❌ Unknown test type: ${options.testType}${colors.reset}`)
            showHelp()
            process.exit(1)
        }
    }
